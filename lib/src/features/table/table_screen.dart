          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ...
                Text('Récapitulatif des scores :', style: TextStyle(fontWeight: FontWeight.bold)),
                ...matchProvider.scoresManches.asMap().entries.map((entry) {
                  return Text('Manche ${entry.key + 1}: ${entry.value}');
                }),
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Text(
                'Manches: ${matchProvider.manchesGagneesTeam1} - ${matchProvider.manchesGagneesTeam2} (Manche ${matchProvider.manche})',
  ...
  Widget _buildPlayersRow(BuildContext context, MatchProvider matchProvider) {
    ...
    return Row(
      ...
      children: matchProvider.isSideSwapped
          ? [Expanded(child: team2Widget), _buildChangeSideButton(matchProvider), Expanded(child: team1Widget)]
          : [Expanded(child: team1Widget), _buildChangeSideButton(matchProvider), Expanded(child: team2Widget)],
    );
  }
  ...
  Widget _buildManchesResult(BuildContext context, MatchProvider matchProvider) {
    return Column(
      ...
      children: [
        ...
        DataTable(
          columns: const <DataColumn>[DataColumn(label: Text('Manche')), DataColumn(label: Text('Score'))],
          rows: List<DataRow>.generate(
            matchProvider.scoresManches.length,
            (index) => DataRow(cells: <DataCell>[DataCell(Text('${index + 1}')), DataCell(Text(matchProvider.scoresManches[index]))]),
          ),
        ),
      ],
    );
